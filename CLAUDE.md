...
| `PH-1.10` | Permission guard + scope decorator at the data layer                                                              | `1.9`          |  A   |   1 |   ⬜   |      — | Forgotten `where` cannot leak. **DESIGN DECIDED IN ADVANCE — see `STATUS.md`:** branded type first, NOT a lint rule (`BR-1849` — pattern matching reports full coverage while missing the query that leaks). If the branded type constrains every repository method, state the cost and ask. If neither lands, say **runtime only** in those words. |
