...
## 5b. Phase 1 Task Queue — live status

> Machine-checked by `pnpm check:ledgers`, the same as `§5`. Added at `PH-1.9` closing (`SB-44`):
> Phase 1's progress figure had been maintained by hand and computed by nothing, which is
> exactly what `BR-1832` says drifts. It is also what the `REMOVE-AT` check needs in order to
> know whether a named task has completed.

| ID        | Task                                                                                                              | Depends        | Type | Est | Status | Actual | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------- | ----------------------------------------------------------------------------------------------------------------- | -------------- | :--: | --: | :----: | -----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PH-1.1`  | Schema: **`roles`** `users` `user_identities` `refresh_tokens` `verification_tokens` `otp_codes` `login_activity` | `0.6`          |  A   | 1.1 |   ✅   |    0.6 | Migration applied, seeds run                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `PH-1.2`  | Argon2id hashing, breach-list check, password policy                                                              | `1.1`          |  A   | 0.5 |   ✅   |   0.35 | ~100 ms hash time verified                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `PH-1.3`  | JWT access + refresh rotation with family reuse detection                                                         | `1.1`          |  A   |   1 |   ✅   |    0.6 | Reuse revokes the family (tested)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `PH-1.4`  | Email registration + verification + password reset                                                                | `1.2`, `1.3`   |  A   |   1 |   ✅   |   0.75 | Enumeration-resistant responses                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `PH-1.5`  | Google OAuth with PKCE and `id_token` verification                                                                | `1.3`          |  A   | 0.5 |   🟡   |    0.8 | Google credentials — awaiting a real round trip                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `PH-1.6`  | `SmsProvider` abstraction + Twilio Verify + phone OTP behind a flag                                               | `1.3`          |  A   |   1 |   🟡   |      — | Twilio credentials; PHONE_OTP_ENABLED stays false                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `PH-1.7`  | Schema: `permissions` `role_permissions` `user_permission_overrides`                                              | `1.1`          |  A   | 0.5 |   ✅   |    0.4 | Migration applied                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `PH-1.15` | Entitlement engine: resolve, additive merge, expiry at read time, Redis cache                                     | `1.14`         |  A   | 1.5 |   ⬜   |      — | p95 < 20 ms                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `PH-1.16` | Quota entitlements with atomic consumption                                                                        | `1.15`         |  A   |   1 |   ⬜   |      — | Concurrent requests cannot overrun                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `PH-1.17` | Schema: `products` `product_prices` `product_entitlements` `coupons`                                              | `1.14`         |  A   | 0.5 |   ⬜   |      — | Migration applied                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `PH-1.18` | Admin: product editor + entitlement composer with live preview                                                    | `1.17`, `1.13` |  A   | 1.5 |   ⬜   |      — | New offer in < 15 min (`MET-05`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `PH-1.19` | Coupon engine: constraints, redemption tracking, friendly errors                                                  | `1.17`         |  A   |   1 |   ⬜   |      — | Specific reason on invalid code                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `PH-1.20` | Schema: `orders` `order_items` `transactions` `invoices` `refund_requests` `subscriptions`                        | `1.17`         |  A   |   1 |   ⬜   |      — | Migration applied                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `PH-1.21` | `PaymentProvider` abstraction                                                                                     | `1.20`         |  A   | 0.5 |   ⬜   |      — | Interface with two implementations stubbed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `PH-1.22` | Stripe: checkout, webhooks, subscriptions, customer portal                                                        | `1.21`         |  A   | 1.5 |   ⬜   |      — | Real payment grants an entitlement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `PH-1.23` | Idempotent webhook pipeline: verify → dedupe → enqueue → 200                                                      | `1.22`         |  A   |   1 |   ⬜   |      — | Duplicate events produce one grant                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `PH-1.24` | Checkout flow UI: summary, coupon, method selection, success routing                                              | `1.22`, `0.27` |  A   |   1 |   ⬜   |      — | Routes into lesson 1 on success                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `PH-1.25` | Invoice generation (queued PDF) with gapless numbering                                                            | `1.20`         |  A   |   1 |   ⬜   |      — | Sequential numbers, stored on R2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `PH-1.26` | Paymob: cards, wallets, installments                                                                              | `1.21`         |  A   | 1.5 |   ⬜   |      — | Real EGP payment succeeds                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `PH-1.27` | Fawry deferred flow + reference display + 72h expiry                                                              | `1.26`         |  A   |   1 |   ⬜   |      — | Payment hours later still grants                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `PH-1.28` | Reconciliation job for pending orders every 15 min                                                                | `1.27`         |  A   | 0.5 |   ⬜   |      — | Lost webhook recovered                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `PH-1.29` | Subscription lifecycle: renewal, grace, retries, pause, cancel routing                                            | `1.22`         |  A   | 1.5 |   ⬜   |      — | Reason-gated remedy works                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
>
> **The guard needs a route table to attach to, and no route table exists until `PH-1.12`.** Closing
> `PH-1.10` now would mark as done a mechanism that authorizes **zero requests** — which is the exact
> shape of a fitness function that loads and enforces nothing (`BR-1830`). The difference between
> "built and wired" and "built" is invisible in a green gate and total in production.
>
> Consequently: **`PH-1.10` stays 🟡 with both gaps named and never reaches ✅**, the same as `PH-0.9`.
> It does not count toward the 7. `PH-1.12`'s scope grows to include registering `PermissionGuard`
> globally and building `BR-1631`'s startup check, and its estimate goes **1 → 1.5** — `PH-1.10`'s
> undelivered half, carried rather than absorbed. **`PH-1.10`'s own Est stays at 1 against an actual of
> 0.7**: rewriting a past estimate to match what was delivered is how the `DEC-56` ratio stops meaning
> anything, so the record reads "estimated 1, delivered half of it in 0.7" — which is the true statement.
>
> The scope move is recorded as a divergence in `STATUS.md §7` and in `16 §PH-1.12`, which is a
> **founder-authorised** edit to a frozen document (`§1`, `BR-1765`) — instructed, not taken here.
...
☐ The fitness cases are RE-AUDITED at exit against `BR-1849`'s criterion — every case's
    ...
    **THE FIRST SCHEDULED AUDIT IS PHASE 1 EXIT, NOT PHASE 2** (founder, 2026-07-31). Three
    consecutive sessions each produced a defect in the VERIFICATION layer — `PH-1.7`'s case 40
    asserting on its own command line, the `check-ledgers` blind spots, and `SB-46`'s suite
    injecting a defect and then certifying it. Every one was found by something downstream
    breaking rather than by looking. **Three instances in three sessions is a rate, not a
