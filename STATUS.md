...
**`josamacademy.com` serves over HTTPS**, behind a proxied Cloudflare record. **The database is
backed up daily to R2 and the restore is verified weekly into a clean database** — verified, not
configured. An uptime monitor pushes to the founder's phone when the site stops answering, proven by
stopping it.

**What is not true yet:** the uptime monitor watches the **public site**, not `/health`, so a failed
database, a stopped Redis or a stale backup will **not** alert — `/health` returns 200 while degraded
by design and nothing reads that field (`SB-34`). Certificate expiry is unmonitored and the
Cloudflare TLS mode is on Automatic, which together are a latent outage (`SB-35`). The Coolify
dashboard is still reachable from the internet on port 8000 (`SB-22`, `PH-0.8` deferred), so
`BR-1702` is **not met**.

**If the server were lost this morning, the database could be restored** — which was not true
yesterday, and is the single largest change in this project's risk position so far.
...
| **Updated by**     | AI (census: 193 tasks, no duplicates; `16`'s count corrected in 5 places and now machine-checked; `BR-1852` allocated)                                                                                                                                                                        |
| **Current phase**  | **Phase 1 — Identity, Permissions, Money** · **7 / 33 CI-verified** · `PH-1.5`/`PH-1.6` 🟡, `PH-1.10` 🟡 permanently. Phase 0 closed at 29/30.                                                                                                                                                |
| **Current task**   | None in progress. `PH-1.10` closed 🟡 — its two gaps are now `PH-1.12`'s scope.                                                                                                                                                                                                               |
| **Next task**      | `PH-1.11` — capability interceptor (`_can` + `_reason`). Then `PH-1.12`, which now carries the guard wiring and `BR-1631`'s startup check as well as the permission matrix.                                                                                                                   |
...
| 1 — Identity & Commerce |      33 |      7 | 🟡 In progress  |
...
> numerators in `CLAUDE.md §5`/`§5b`. It had drifted in three places before that: Phase 0 read 28
> against `§5`'s **29**, Phase 1 read 0 against `§5b`'s **7**, and its rows summed to 192.
>
> **The 193 was correct and is now derived.** The census (2026-08-01, founder-authorised) found
> `docs/16` contains **193 rows, 193 distinct ids, no duplicates and no gaps**. The earlier "196
> rows / Phase 0 showing 34" was a grep artefact of mine, not a property of the document — the
> pattern also matched an estimate-split discussion table. `docs/16`'s own **191** was the wrong
> number: stated at its first commit when the file enumerated 190, never recomputed through three
> task additions, and briefly correct by coincidence. Corrected to 193 in all five places.
...
| Gap                                                     | Owner                                                           |
| ------------------------------------------------------- | --------------------------------------------------------------- |
| **A degraded `/health` does not alert**                 | `SB-34` — the monitor watches the public site, not `/health`    |
| **Certificate expiry unmonitored + TLS mode Automatic** | `SB-35` — a compound latent outage                              |
...
| ~~Alert recovery notification unconfirmed~~             | ✅ **Confirmed received 2026-07-30.** Criterion 4 fully closed. |
...
### 2026-07-30 · PH-0.28 — ✅ EXECUTED. Backups, restore verification and alerting live.

**By:** Founder executed; AI authored
**Time:** 0.7 d authoring + execution (founder has not reported the execution figure yet)
**Criteria:** closes exit criteria **3** and **4**; completes **9** for everything Phase 0 owns.

### What was proven

**Backup (§4).** Dumped 2,478 bytes and reported `uploaded and verified` — the line that matters,
because the script re-reads the object's size from R2 rather than trusting `aws s3 cp`'s exit code.
**Refusal (§4.1):** a bad database name failed at `pg_dump` and uploaded nothing.

**Restore verification (§5) — exit criterion 3.** `PASSED`, into a clean database, with
`pg_restore` completing with no errors and **`ledger intact — 1 applied migration(s) survived the
round trip`**. No `restore_check_*` left behind (§5.1), and with an empty prefix it correctly
**FAILED — no backup found** (§5.2). Both directions observed, which is what separates a
verification from a configuration.

**Scheduled (§6):** `17 2 * * *` and `41 3 * * 0`, staggered so the weekly check reads a fresh
object rather than racing the upload.

**Health (§9) — criterion 9 for everything Phase 0 owns:**
...
```json
{
  ...
  "checks": {
    "database": "ok",
    "redis": "ok",
    "last_backup": { "status": "ok", "dump_age_hours": 0.1, "verified_days_ago": 0 }
  },
  "version": "a4c3d43c7fa8acd9cc6d8e20521d01e51f3f5bb4"
}
```

**`version` reads the SHA, not `0.0.0` — `PH-0.11` divergence 1 is closed** on a real deploy, so
criterion 2's proof no longer rests on image-tag inspection.

**Alert (§8.3) — exit criterion 4.** Stopping the web container produced a 503 confirmed from four
UptimeRobot locations over 46 seconds, then a push to iPhone and an email. Restarted; back to 200.
**The recovery notification was confirmed received on 2026-07-30.** Criterion 4 is fully closed —
both transitions observed, not just the one that fires. `BR-1836`'s lesson generalised: the down
alert is the half everyone tests and the recovery half is the half that latches.

**Domain — `PH-0.11 §10`, done today rather than skipped.** `josamacademy.com` serves over HTTPS
behind a proxied Cloudflare record, all five route groups 200 **from the public domain**, and CSS at
**27,205 bytes — identical to the host-port figure**, so `PH-0.30`'s base-surface fix holds through
the proxy. Client sites checked immediately after the proxy reload: `404/404/200/404`, unchanged.
**`SB-32` closes.**

### Five divergences

**1 — the monitor is plain HTTP, not a keyword monitor.** UptimeRobot only offers the type at
creation, and `/health` is not publicly reachable. **Consequence is real and is recorded as a gap,
not as covered:** a failed database, a stopped Redis or a stale backup will not alert, because
`/health` returns 200 while degraded by design and nothing reads that field. Criterion 4 as written
is met; this is the gap beyond it. `SB-34`, with the recommended fix — a Next route proxying
`/health` server-side, which needs no new exposed surface.

**2 and 4 — certificate expiry unmonitored, TLS mode on Automatic.** Recorded together as `SB-35`
because **they are only dangerous together.** Alone, a failed renewal is survivable: SSL mode
**Full** encrypts to the origin without validating its certificate, so the site keeps answering —
which is also why nobody would notice. The danger is Automatic mode moving to **Full (strict)**,
which does validate, taking the site down with no warning while expiry is unwatched. One fix removes
both: a **Cloudflare Origin CA certificate** on the origin and the mode pinned to Full (strict).

**3 — two `A` records, and the failure looked like success.** A leftover Hostinger parking record
sat alongside ours; Cloudflare round-robined between them, so the domain served the parking page
roughly half the time **while `curl` reported 200**. Deleted. Added to `PH-0.11 §10.1`: after adding
an `A` record, check for **others on the same name** — a second record does not replace, it
load-balances, and a single sampling check cannot see it. Same family as `BR-1839`: a confident,
truthful signal that answers a question nobody asked.

**5 — a client container restarted a minute before the deploy**, `RestartCount 0`, a separate
redeploy. All four client sites verified unchanged throughout. Recorded so the uptime anomaly in the
...
### 2026-08-01 · The census: no duplicates, no hidden tasks — and my 196 was wrong

**By:** Founder (authorised the census and the `/docs` correction) · AI (executed)
**Calendar: 2026-08-01 -> 2026-08-01 (1 day).**

#### The correction first

**I reported "196 rows, Phase 0 showing 34 against 30 real tasks" and hypothesised duplicates.
Both figures were artefacts of my own grep, not properties of the document.** The pattern was
`^| \`PH-` — first cell *begins with* a task id — which also matches the estimate-split discussion
table, whose first cells read `` `PH-0.8` Cloudflare Tunnel ``. Four such rows, all Phase 0. That
is the entire 34-vs-30 gap and four of the five rows in the 196.

Counted properly — first cell **exactly** `` `PH-N.M` `` — `docs/16` contains **193 rows, 193
distinct ids, no duplicates, and no gaps** in any phase's numbering. The founder's concern was the
right one to raise and the answer is the reassuring branch of it: no task is counted twice, and no
task is hidden behind a reused id. **The remaining work is what it appeared to be.**

#### What was actually wrong, which is worse than a duplicate

`docs/16` stated **191 tasks** and enumerated **190 at its own first commit** — `f6dcc04`, before a
single task was executed. Traced through history:

| commit    | enumerated | stated | what happened                          |
| --------- | ---------: | -----: | -------------------------------------- |
| `f6dcc04` |        190 |    191 | wrong from birth                       |
| `8e07222` |        191 |    191 | `PH-0.29` — **correct by coincidence** |
| `43ddbb9` |        192 |    191 | `PH-0.30`                              |
| `e1278fb` |        193 |    191 | `PH-1.33`                              |

**The middle row is the dangerous one.** For one commit the number was right, and anybody who
checked it then would have confirmed it and moved on. A figure that is right by accident is worse
than one that is plainly wrong — `BR-1849`'s direction-of-failure argument applied to a count.

Three separate task additions each moved the enumeration past a figure nothing recomputed. **The
per-phase headings drifted the same way**: Phase 0 read 28 against 30, Phase 1 read 32 against 33.
Phases 2–7 were exact, which is why nobody caught it — the file looked internally consistent
wherever anyone happened to look.

**`STATUS.md §1`'s 193 was right all along.** Not because it was derived — it was not — but
because it had been maintained by hand against a different mental model. Two undeived numbers
disagreeing, and the one nobody trusted was the correct one.

#### The corrected census — computed, not asserted

| Phase | 0   | 1   | 2   | 3   | 4   | 5   | 6   | 7   | **Total** |
| ----- | --- | --- | --- | --- | --- | --- | --- | --- | --------- |
| Tasks | 30  | 33  | 34  | 26  | 22  | 18  | 16  | 14  | **193**   |

Five stale figures corrected in `docs/16` (founder-authorised): the `Contains` header row, the
Phase 0 and Phase 1 headings, the `Totals:` line, and the approval-table row — all three of the
grand-total mentions, not only the two named, because a third stale copy of the same number would
have re-seeded the drift the moment the check was written.

#### The check I wrote to guard the census got the census wrong

**First run: it enumerated 192 and disagreed with a count I already had.** Cause: I reused
`parseTaskRows`, which requires a table header so it can resolve `Status`/`Est` **by name** — and
`PH-0.30` sits in a **headerless one-row table embedded in prose**. The strict parser dropped it
silently.

Reusing a parser built for a different job is how a check acquires a blind spot it did not intend
and cannot see. The census needs **ids only**, so it now has its own scan with an exact-match anchor
— which also excludes the discussion-table rows that started this. **It was caught only because two
independent measurements disagreed.** One measurement is a number nobody can check; that is the
whole thesis of this file, and it applied to me while I was implementing it.

The census check also **fails on a duplicated id** — the founder's hypothesis, now permanently
guarded even though today's answer was clean.

**Proven by six deliberate injections, all caught** (`BR-1835`): a drifted phase heading, a drifted
grand total, a duplicated id, a `STATUS §1` per-phase disagreement, a `STATUS §1` Total that
disagrees with its own rows, and `STATUS §1`'s Done column disagreeing with `CLAUDE.md`'s
machine-checked numerator — the last being the drift that actually happened (28 vs 29, 0 vs 7).
Two of the six are now permanent fitness cases, **50** and **51**.

#### `SB-46`'s preflight: the first real save, four days after it was written

**It stopped me committing an injected schema.** My own 2-minute timeout killed the pre-commit
hook's `verify:fitness` run partway through case 47, leaving `ScopeProbe` appended to
`apps/api/prisma/schema.prisma` and a scatter of `__violation` files across four packages. `git
status` looked clean at several points — lint-staged restores state after a failed hook — and the
tree was in fact not clean.

**The preflight refused to start, four times, until every artefact was gone.** Without it the
commit would have carried a fabricated Prisma model into `main`, where the next task would have
found it as an unexplained table in a schema that `PH-1.10` had just proven deny-by-default.

Written 2026-07-31 at a cost the founder questioned as overhead; paid for itself on 2026-08-01. The
mechanism that prevents a defect never gets credit for the incident that did not happen — this one
is written down because it can be.

#### `BR-1852` allocated

**A corpus that contains the value it forbids will flag itself; a marker-scanning check must BUILD
the marker, never spell it.** In `12 §19.1` beside `BR-1849`, per founder instruction, with the
case-49 worked example. `BR-REGISTRY §4` advanced to `BR-1853`; `§3`'s range for `12` corrected to
`BR-1830`–`BR-1852` (`BR-1843` skipped).

**Still numberless:** the unreachable-expiry rule from 2026-07-31, recorded in `CLAUDE.md §4`. It
is the neighbour of `BR-1852` in both subject and origin and could be `BR-1853` in one line — not
taken, because the authorisation named one rule.

**Also not derived:** `BR-REGISTRY §3`'s count column says **375** for `12` where **376** rules are
bullet-defined. Range corrected, count left as found — recomputing one cell beside eleven uncounted
ones would make the table read as verified when it is not.

---

### 2026-07-31 · The unreachable expiry, generalised — and the checker had the defect it checks for

**By:** Founder (three decisions) · AI (implemented)
**Calendar: 2026-07-31 -> 2026-07-31 (1 day).**

Follows directly from the entry below. Recording the general form turned up a second instance
immediately, in the checker itself.

#### 1. The general form is now a rule and two fitness cases

**A marker whose expiry condition depends on a task's STATE can become permanently unreachable when
that state changes for an unrelated reason.** Nothing edits the marker; it stops being able to fire
and goes on reading as governed. **Any `REMOVE-AT` marker naming a 🟡 task is already dead** — amber
here is not "resolving shortly", it is a state this project deliberately parks tasks in, sometimes
permanently.

`check-ledgers` now fails on ✅, 🟡 **and** unknown owners. Unknown was not asked for; it is the same
defect (an expiry that can never arrive) and costs three lines, so it is in — say the word and it
comes out. Proven by **fitness cases 48 and 49**, not by a one-off probe: a proof that re-runs only
when someone remembers is not a safety net (`BR-1830`).

Deliberately **not** applied to `12 §19` deferral owners. A deferral names who will _activate_ a
check, and naming an in-progress task there is legitimate — row 20 names `PH-1.10` on purpose. A
`REMOVE-AT` says "delete me when X finishes"; a deferral says "X owes this". The two look alike and
are not, and the asymmetry is commented in the checker so it reads as a decision rather than a miss.

Recorded in `CLAUDE.md §4`. **No `BR` number allocated** — its home is `12 §19.1` next to
`BR-1849`–`BR-1851`, a frozen document not covered by this session's authorisation. One word makes
it `BR-1852` plus a registry update (`BR-1821`).

#### 2. `packages/contracts` had no owning task — that was the defect under row 19

Row 19 named `PH-1.8`, which is **done** and which built the permission registry, not contracts.
Re-pointing it was not the fix, because **nothing in `16` ever created `packages/contracts`** while
three separate rules assumed it existed: this row (`BR-1447`), `BR-931` ("the single source of truth
for API types"), and `BR-1584` ("generated in CI; hand-editing fails the build"). A row cannot be
owned into existence.

**`PH-1.33` added to `16`** — founder-authorised, the second such edit today. `packages/contracts`
generated in CI from the NestJS OpenAPI surface (`DEC-30`, `@nestjs/swagger`, already pinned at
11.4.6 in `13` and marked "Phase 1 — contracts generation"), hand-edits failing the build. **Depends
`1.12`**, because generating from the OpenAPI surface requires that surface to exist — which is the
founder's placement, and the same reasoning that moved the guard there. **Est 1.5**, sized against
`PH-0.16` (fitness functions, 1.5) rather than the 1-day schema tasks: it is a new package, a codegen
pipeline, a CI step, a build-failing hand-edit guard, and a fitness case.

Row 19 now reads ⬜ `PH-1.33`. Phase 1 goes **7 / 33**.

#### 3. The checker's owner set was Phase 0 only — row 15's defect, inside the check for row 15

`check-ledgers` built its completion set from the `PH-0.x` rows alone, so **every Phase 1 owner
marked done passed silently.** Row 19 sat there, orphaned on a completed task, invisible to the
mechanism written to catch exactly that.

Fixed **generally**, per founder instruction: no phase is named anywhere. Any table with an
`ID`/`Status` header and `` `PH-N.M` `` in its first column is a task table, whatever N is. Columns
resolve **by header name**, because `§5` and `§5b` order theirs differently — which is why the old
code needed two regexes and therefore a per-phase list at all.

**A per-phase list has the defect built in.** It is correct until a phase starts and quietly wrong
afterwards, and the extension is invisible work nothing demands. It would have reproduced at Phase 2
for the identical reason.

Rejected the obvious alternative — "find the cell containing a status glyph" — because it is already
wrong today: `PH-1.10`'s Note cell contains both 🟡 and ✅ in prose and would be read as the status
column. Added a duplicate-id guard while there: one task asserted about in two tables is two tables
that might disagree.

**The widened checker caught row 19 on its first run**, before I had touched `STATUS.md`. That is
the proof the widening is load-bearing, and it arrived unprompted.

#### What is NOT resolved

**`docs/16`'s task census does not reconcile with its own tables, and adding `PH-1.33` did not cause
it.** The header and totals lines both say **191 tasks**; the file contains **196 rows** matching
`` `PH-N.M` `` (Phase 0 alone shows 34 against `CLAUDE.md §5`'s 30, so some rows appear in more than
one table). `STATUS.md §1` says 193. Three numbers, none computed from anything. Same class as
`SB-08`'s 134-vs-151 component census, and it needs a founder-authorised recount — not a guess from
here. `§1`'s per-phase rows are reconciled below against the machine-checked `CLAUDE.md` figures;
the **193 total is left alone** because it is exactly the number in dispute.

`12 §19` row 20 left for `PH-1.12`'s close, as agreed — whether cases 46/47 already satisfy
`BR-918` is a judgement on evidence, not a re-point.

---

### 2026-07-31 · `PH-1.10`'s two gaps are re-owned to `PH-1.12` — founder decision

**By:** Founder (decision) · AI (recorded)
**Time:** no execution — a scope and ownership decision, recorded in the same commit as the tables it changes
**Calendar: 2026-07-31 -> 2026-07-31 (1 day).**

**The decision.** The two mechanisms `PH-1.10` did not build — `PermissionGuard` wired in, and
`BR-1631`'s startup check — **close under `PH-1.12`, not under `PH-1.10`.**

**The reasoning, which is the part worth keeping.** The guard needs a **route table to attach to, and
no route table exists until `PH-1.12`.** Closing `PH-1.10` now would mark as done a mechanism that
**authorizes zero requests** — the same shape as a fitness function that loads and enforces nothing
(`BR-1830`). Three of those were shipped in this repository already, and every one looked complete
from the outside. A guard registered nowhere passes every test written against it, because the tests
instantiate it directly; the gap is not in the unit, it is in the wiring, and nothing in a green gate
...
| Where                     | Change                                                                                                                                |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `CLAUDE.md §5b` `PH-1.10` | Note names both gaps and their new owner. **Status stays 🟡 and never becomes ✅** — the `PH-0.9` precedent.                          |
| `CLAUDE.md §5b` `PH-1.12` | Task text gains guard wiring + startup check. **Est 1 → 1.5.**                                                                        |
| `docs/16 §PH-1.12`        | Same, in the specification. Output column now names three proofs, not one. Refs gains `BR-1631`. **Founder-authorised `/docs` edit.** |
| `STATUS.md §7`            | Divergence recorded.                                                                                                                  |

**`PH-1.10` does not count toward the 7 / 32.** It closes amber, permanently, with its gaps named —
which is the honest reading and the one `BR-1804` asks for. A task that delivered half its output is
not 0 and is not 1, and the queue has a glyph for exactly that.

**The estimate is carried, not absorbed.** `PH-1.12` goes 1 → 1.5; `PH-1.10`'s Est **stays at 1**
against an actual of 0.7. Rewriting a past estimate to match what was delivered would make the
`DEC-56` ratio meaningless — the record has to be able to say "estimated 1, delivered half of it in
0.7", because that is what happened. Phase 1's estimated total rises by 0.5. Real cost, recorded.

#### The decision created a permanently-silent check, and the probe is what found it

**A "never ✅" task cannot own a `REMOVE-AT` marker.** `.dependency-cruiser.mjs` exempts
`packages/abilities/src/index.ts` from the orphan rule and carried `REMOVE THIS LINE AT PH-1.10`.
`check-ledgers` fires that marker only when its named task is **DONE** — so the moment `PH-1.10`
became permanently 🟡, the expiry condition became **unreachable**. The suppression would have sat
there looking governed, with the mechanism that governs it structurally unable to speak. That is the
`SB-44` shape one level over: not a number nobody recomputes, but an expiry nobody can trigger.

Re-pointed to **`PH-1.11`** — verified first that **nothing imports `@josam/abilities` today**, so
the exemption still earns its place and this is a re-owning, not a removal. `PH-1.11` is ⬜, which
`BR-1833`/`BR-1840` require of any owner.

**It was found by probing `check-ledgers`, not by reading the file.** Flipping `PH-1.10` to ✅ to
confirm the numerator check was live printed a second failure nobody asked for — the orphaned marker.
Neither the decision nor a careful read of `.dependency-cruiser.mjs` would have surfaced it, because
the defect is not in either file: it is in the **relationship** between a status glyph in `CLAUDE.md`
and a comment in a config, and only the checker holds both. Deliberate failure earning its keep by
reporting something outside the question asked (`BR-1835`).

#### Why case 46's escape-hatch reasoning is in the file rather than in a commit message

Founder instruction, and it generalises past this task:

> the twentieth looks exactly like the first, and nothing in a diff separates "this genuinely runs
> before authentication" from "scoping this was awkward"

That is why the pin at 18 is **visibility, not prevention**, and why raising it must be a deliberate
edit to `scripts/check-scope-exceptions.mjs` in the same commit as the call it admits. The mechanism
does not decide whether an `unscoped()` is justified — it cannot, both cases are one identical line.
It makes the _count_ move in a diff, where a reviewer reads it. Every escape hatch this project adds
should be held to the same test: not "can it be abused" but "is the abuse visible in a diff".
...
it required closing `SB-44` first — deciding whether a suppression has outlived its reason means
knowing whether the named task is finished, and for a **Phase 1** task nothing knew.

### `CLAUDE.md §5b` — the Phase 1 queue

32 rows, **derived from `16` rather than hand-transcribed**, with the same shape `§5` uses so the
existing checker covers it with no second parser. `check:ledgers` now recomputes
`Progress (Phase 1)` from the table.

That closes `SB-44`, which had recorded the real gap the ledger recount exposed: Phase 1's figure
was maintained by hand and **computed by nothing**, which is exactly what `BR-1832` says drifts —
and it had never been computed at all.

### The `REMOVE-AT` check

Any `REMOVE-AT-PH-x.y` marker in `.dependency-cruiser.mjs`, `turbo.json` or `verify-fitness.sh`
whose named task is **done** fails the build. One line of reuse: the same completion map the
deferral check already builds, now spanning both phases.

**Why it earns its place.** This repository has already produced one exception outliving its
reason — `12 §19` row 15, orphaned against a closed task, which read as _scheduled_ and would
never have been actioned by anyone. A marker records why a suppression exists and when it stops
being justified; without a check it is a comment nobody re-reads.

### Three probes, all caught

Phase 1 numerator drift · Phase 1 denominator drift · a marker naming a completed task. And the
new check caught **my own** off-by-one immediately — I read the status from the estimate column and
it reported `numerator is 7, table has 0 rows marked done` on the first run.

---

### 2026-07-31 · PH-1.9 — `packages/abilities`, shared CASL definitions

**By:** AI · **Calendar:** 2026-07-31 · **Time:** estimated 1.0 d → actual 0.5 d
**Status:** committed, **awaiting CI**.

`defineAbilitiesFor(actor)` per `05 §8`, consumed by the API, web and mobile alike (`BR-708`,
`FEAT-018`). CASL 7.0.1, the version `13 §18.1` pins for this task. 14 specs.

### The ordering IS the rule, so it is asserted as one

`BR-038`'s resolution order is **revoke → grant → role → deny**, and in CASL the last matching
rule wins — so every `cannot` is added after every `can`. That is the implementation of the
precedence, not an incidental detail: **reverse the two loops and a revoke silently becomes a
no-op** whenever a later grant touches the same model.

The spec asserts it survives permission ORDER, not merely that a single revoke works. A test with
one grant and one revoke passes under both implementations.

**A revoke is unscoped on purpose.** `05 §5`'s override table revokes a capability outright, so a
scoped revoke would leave the actor able to act on everyone _else's_ records — the inverse of the
intent. Asserted directly.

### Three deliberate choices

- **`parsePermission` throws on a malformed key** rather than returning a partial parse. A partial
  parse produces a rule matching nothing, which reads as _"this actor lacks the permission"_ — a
  denial nobody can trace back to a typo. Failing loudly is `BR-1849`'s direction.
- **`super_admin` short-circuits before the loops** (`BR-963`, `BR-639`). Its permissions are
  implicit and never stored, so an empty array must not mean a total lockout for that one role —
  and a revoke must not touch it, because implicit authority is not row-derived.
- **`AppAbility`'s subject widened to `string | Record<string, unknown>`.** `MongoAbility<[string,
string]>` types conditions as `MongoQuery<never>`, so `can('update', 'course', { owner_id })`
  will not compile at all. Naming the record shape is what makes `.own` scoping expressible.

### `BR-708` is enforced structurally, and I checked rather than assumed

A general `packages-must-not-depend-on-apps` rule already existed and _should_ have covered the new
package. **"Should cover" is the assumption this session has punished repeatedly**, so I added an
`apps/api` import to `packages/abilities` and confirmed dependency-cruiser errors on it, then
removed it. It does.

That also surfaced a `no-orphans` warning — nothing imports the package yet. Added to the existing
exception list beside `packages/ui`'s, **with an explicit REMOVE-AT-`PH-1.10` note**: an exception
that outlives its reason is how a warning list stops meaning anything.

---

### 2026-07-31 · PH-1.8 — Permission registry, startup sync, orphan flagging

**By:** AI · **Calendar:** 2026-07-31 · **Time:** estimated 1.0 d → actual 0.7 d
**Status:** committed, **awaiting CI and a deploy** (`BR-1846` — this changes composition).

**174 permissions across 18 module groups**, and every module's count matches `05 §5`'s summary
table independently — 11, 9, 17, 7, 25, 9, 6, 12, 5, 8, 7, 5, 9, 6, 8, 7, 9, 14. That is a second
check on the extraction rather than a restatement of it.

### The registry drifts in neither direction

`permission-registry.ts` is the single source file (`BR-709`). `permission-sync.spec.ts`
re-derives the list from `05` and compares **both ways**: a permission in code but not in `05` is
undocumented; one in `05` but not in code is unimplementable. Nothing else catches either.

### The API refuses to start on a bad sync — and why that is the cheap option

**A partial registry does not degrade access, it silently REVOKES it.** `PH-1.10`'s guard resolves
_deny_ for any permission it cannot find, so a half-written table produces an administrator who
can no longer administer, with the symptom three layers from the cause and nothing pointing at the
sync. Refusing to boot converts that into a **deploy failure that names itself** — the same trade
as `JWT_SECRET` at `PH-1.3`.

One transaction, orphan-flagging included. Two guards run **before** it opens: a registry whose
length disagrees with `05`, and duplicate keys. Both throw, and both are asserted to leave the
table untouched.

### `BR-964` — flagged, never deleted, and the reason is `ON DELETE CASCADE`

`role_permissions.permission_id` cascades. Deleting a permission row therefore **silently drops
every grant referencing it**, so a permission temporarily removed from code — a refactor, a bad
merge, a feature flag — would destroy an administrator's configuration permanently, and restoring
the code would **not** restore the grants. Flagging is reversible; deletion is not. Reinstatement
is the other half, and the round trip is asserted lossless.

**Fitness case 45** proves it by deliberate violation: the sync is rewritten to `deleteMany` the
orphans, and the spec asserting the row survives must fail. `BR-1725` — enforcement is proven by
breaking it, not by a green spec. Audited against `BR-1849`'s criterion before being trusted: its
pattern cannot match its own command string.

### One of my own tests was fake, again

`throws when the registry count disagrees with 05` originally built an **inline copy of the
check** and asserted against that, because the guard was unreachable — it read `PERMISSION_REGISTRY`
directly. The test passed whatever the service did. The registry is now a parameter, the guard is
reachable, and the spec exercises it. Third instance this session of a test that could not observe
the property it named.

### Descriptions are deliberately absent — `SB-45`

`TBL-008.description` is nullable bilingual JSONB, and `BR-953` requires `ar` when present. `05`
carries English only. Writing 174 invented Arabic translations into the database the primary
audience reads is worse than a null, and precisely what `BR-524` exists to prevent.

### Verified

`45 caught, 0 NOT caught` · 12 specs · the API boots and logs
`permission sync: 174 active (+0 new, 0 reinstated, 0 newly orphaned)`.
...
**Nothing moved — and two parts of the premise were wrong, which matters more than the null
result.**

| Figure                            | Old (`.includes`) | New (closed set) | Changed? |
| --------------------------------- | ----------------- | ---------------- | -------- |
| `CLAUDE.md §5` progress numerator | **29**            | **29**           | no       |
| `12 §19` active rows              | **17**            | **17**           | no       |

**Why it never fired:** every status cell in `CLAUDE.md §5` holds exactly one glyph and no prose —
`✅` or `⏸️`. Two glyphs in one cell is the only input that separates the two implementations, and
no cell has ever had them.

**Two corrections to the premise, both mine to make:**

1. **`PH-1.5` and `PH-1.6` were never counted by this check at all.** Its row pattern is
   `PH-0\.\d+` — `CLAUDE.md §5` is the **Phase 0** queue. Phase 1's _5 / 32_ is maintained by hand
   in this file, produced by no checker, and therefore untouched. **It is also, for exactly that
   reason, the figure with no automated guard at all** — the real gap this recount exposed, and
   recorded as `SB-44`.
2. **`PH-0.9` is genuinely `✅`.** Marked done after §6.5 returned `restarts=0, oom=false` on all
   five containers, with founder confirmation. Not a 🟡 being miscounted.

**And a correction to my own report:** I classified the ledger defect as **LIVE** and the other two
as latent. That was wrong. It sits in a load-bearing path — it decides the progress numerator — but
it **never produced a wrong number**. All three were latent. Calling it live overstated a defect in
the direction of alarm, which is the opposite of this project's usual error and still an error.

### `BR-1849` — the sharper half

`NUMERIC(10,4)` → `NUMERIC` would have **PASSED**, not failed: `NUMERIC` lowercased equals the real
`udt_name`, so the assertion would have compared a type it never read. The precision — the part
deciding whether money rounds correctly — would have gone unverified by the suite whose purpose is
verifying it, reporting green.

The `SET NULL` instance _alarmed_ the first time a multi-word action appeared. The `NUMERIC` one
never would have. **Same root defect, opposite observability, and the silent one is strictly
worse.** With the obligation that a defect found in a verifier requires recomputing everything it
ever reported — **including when the answer is "nothing changed", because that answer is only worth
having when it was computed.**
...
### 2026-07-30 · ✅ DEPLOYED — `PH-1.2`–`PH-1.6` code runs on the server for the first time

```
version 9821098de026fee27e87aed10aff21c7c69c3712
checks  database ok · redis ok · last_backup ok (dump 12 h, verified 0.6 d ago)
boot    Nest application successfully started · IdentityModule initialized
        migration 20260730092654_identity_and_roles applied
```

**Two failures on the way, both environment, both now rules.**

### 1. `JWT_SECRET` was missing in Coolify — the container restart-looped

`PH-1.3` made it required with no default. That was correct: a default means every deployment that
forgot it shares one publicly-known signing key. But **nothing connected the code change to the
deployment environment**, so the failure sat queued for three tasks and surfaced on the first
deploy since.

**It cost five minutes instead of an investigation because `BR-943` makes the application fail at
startup naming the variable.** That rule earned its place today.

Now **`BR-1847`**: a task that makes a variable required has created a deploy that WILL fail, and
must name it in its report as an explicit founder action — which variable, where it goes, what
generates a valid value. `CLAUDE.md §2` step 7 carries the obligation. A `.env.example` entry is
necessary and never sufficient, because nobody reads it during a deploy.

### 2. `APP_VERSION` was stale — `/health` reported `a4c3d43` while serving `9821098`

Recorded as **`SB-43`**, recommendation only, per founder direction. **An audit value supplied by
the mechanism it audits cannot detect that mechanism's error.** `version` exists to answer _which
code is actually running_; today it answers _which value somebody last typed into the deploy
configuration_ — the very thing in question when the answer matters. Bake it in at build time from
the commit SHA. It has already cost confusion once, at `PH-0.11`, where the field read `0.0.0` for
all of Phase 0.

### Both confirm `BR-1846`

The stale image hid the `JWT_SECRET` gap from `PH-1.3` until the first deploy since — and would
have hidden any others just as long.

### `SB-41` closed — measured, not estimated

`mean 186 ms · median 196 ms · p95 218 ms` on 2 vCPU, against **161 ms** on the laptop.
**Ratio 1.15×.** Accepted as measured; no retune. The ratio is the reusable output — the box is far
closer to development hardware than its specification suggests, and that multiplier now applies to
every CPU-bound estimate here. `PH-1.4`'s 400 ms enumeration floor still exceeds p95, so `BR-1611`
holds unchanged.

### `SB-42` works as designed

The boot warning fired naming the 64-hash corpus and the 1,000,000 threshold. Visible without
blocking, which was the whole point of writing the assertion inverted.
...
### 2026-07-30 · PH-1.2 — Argon2id, breach list, password policy · ✅

**By:** AI · **Calendar:** 2026-07-30 → 2026-07-30 (same day)
**Time:** estimated 0.5 d → actual 0.35 d

**Output:** `PasswordHasher`, `BreachList`, `validatePasswordStructure`, `SecurityModule`, and
20 specs — **6 of them proven by deliberate failure** (`BR-1835`).

### The measured number, which is the task's stated output

**`argon2id: 161 ms per hash`** on this development machine, against `BR-1607`'s ~100 ms target.
Same order of magnitude, so the parameters are right — but the target is _the production CPU_, and
production is **2 shared vCPU**, which is slower than this box, not faster. Recorded as `SB-41`:
measure on the server before `PH-1.4` puts a login behind it.

The spec asserts an **order of magnitude** (15 ms – 1500 ms), not a tight bound. A tight bound
fails on hardware variance and teaches everyone to ignore the one test that would catch a
parameter set 8× too slow — which produces correct hashes and passes every functional test.

### Three decisions where the specification's silence was the point

- **No maximum password length, and no composition rules** beyond a letter and a digit
  (`BR-1608`). `14 §2.1` gives the reason: complexity rules produce weaker, more predictable
  passwords. A max length is worse still — usually a symptom of storing the password rather than
  a hash of it.
- **`\p{L}` and `\p{Nd}`, not `[a-zA-Z]` and `[0-9]`.** On an Arabic-first platform, a password
  rule that silently rejects Arabic letters or Eastern Arabic numerals (٠-٩) is a defect **nobody
  on an English keyboard would ever encounter**. Both are proven by probe.
- **Code points, not `.length`.** Four astral characters are 8 UTF-16 units, so `'👍👍👍👍'`
  would have passed an 8-character minimum. Proven by probe.

### `BreachList` refuses rather than reporting clean

`BR-1609` / `DEC-48`. The failure this class is shaped around: a breach check that returns
`false` because it has **nothing to check against** is indistinguishable at the call site from one
that genuinely cleared the password — and it would let every known-breached password through while
registration reported success. So `isBreached()` **throws** when the corpus is not loaded, and
`status()` reports `loaded: false`. An empty-but-present file is treated as unavailable too: it is
the failure most easily mistaken for a working check, having none of the symptoms of a missing one.

**SHA-1 is correct here and it is not a security decision.** It is a lookup key into a corpus
published and indexed by SHA-1; confidentiality is Argon2id's job. SHA-256 would key nothing.

### A build defect this task would otherwise have shipped

`tsc` copies `.ts` and nothing else, so `breach-corpus.txt` never reached `dist/`. The compiled
build looked correct and would have **refused every breach check in production** while passing
every test here, because the source tree has the file and `dist/` does not — `BR-1838` exactly.
`scripts/copy-assets.mjs` now carries it, and the fix was **verified by deleting the file from
`dist/` and confirming `BreachList` refuses**, not by observing that the copy step ran.

### Divergences

- **`@node-rs/argon2` 2.0.2 added** — pinned exact after pnpm wrote a caret range (`BR-1591`).
  Chosen over `argon2` because it ships prebuilt binaries and needs no `node-gyp` in the Docker
  build. Argon2id is mandated by `BR-1606`; there is no way to do this task without an
  implementation.
- **`Algorithm.Argon2id` cannot be referenced by name** — it is an _ambient_ const enum, which
  `isolatedModules` rejects (TS2748). Inlined as `2`, and the spec asserts the produced hash
  begins `$argon2id$` — the algorithm the library actually used, rather than the number we asked
  it for (`BR-1837`).
- **The corpus is a 64-entry starter list**, not HIBP's full ~850 M. The mechanism is complete and
  sharded exactly like HIBP's published range files, so loading the real corpus is a deploy step
  with no code change. Recorded as `SB-42`.
...
**Status:** ✅ **DONE 2026-07-30** at 29/30, on §6.5 passing 5/5. §7 and §8 are recorded against
`PH-0.8`, not against this task. `SB-39` carries the open sizing question forward.

**Output:** `docs/runbooks/coolify-hardening.md`, now carrying its own execution record at §11.

### §6.5 — run after several hours, not overnight

Five lines, `restarts=0`, `oom=false` on all five containers.

**The shortened wait costs nothing, and saying why matters more than the result.** `SB-39` already
establishes ~198 MiB in use against 3,008 M allocated. **A day of no load and a few hours of no load
prove the same thing**, because the variable under test — memory pressure — is absent from both.
Waiting overnight would have produced a more impressive record of the same non-event.

**What it confirms is narrow and real:** the limits do not break the containers at rest. A 320 M cap
did not stop Redis running; the API booted under 640 M. That was a genuine risk — the runbook's §9.1
exists for it — and it is closed. **What it cannot confirm is whether the numbers are right**, and no
idle observation of any length can. `SB-39` carries that to `PH-1.15`.
...
to re-examine. **Named owner: `PH-1.15`**, the entitlement engine — the first Phase 1 task with a
measurable performance criterion (`p95 < 20 ms`) that undersized memory would actually violate. The
"empty schema" premise expires earlier, at `PH-1.1`, the first non-empty migration. Recorded as `SB-39`.
...
**§10, attaching the domain.** `josamacademy.com` was not serving on the day, recorded as `SB-32`. It
is the only step that reloads `coolify-proxy`, and the deploy was fully proven without it.

**Completed 2026-07-30** alongside `PH-0.28`. `SB-32` closed — see that entry, and the duplicate-`A`-record
warning it produced.
...
**Time:** estimated 1.0 d → actual 1.1 d

**Verified:** `lint` 10/10 · `lint:hook` · `lint:css` · `format:check` · `check:deps` ·
`check:catalogs` · `check:renovate` · `check:size` · `typecheck` 9/9 · `test` 10/10 · `build` 5/5 ·
`pnpm audit --prod` exit 0 · `verify:fitness` **40 caught, 0 NOT caught** ·
**49 stories x 4 theme/direction combinations, all green.**

---

#### 1. Criterion 6, and the mechanism decision it turned on
...
**Neither axis changes the rendered DOM.** The entire difference between the four combinations is
CSS, and jsdom applies none - so extending the spec-level loop would have asserted the same markup
four times and reported four passes. That is `BR-1830`'s shape exactly, and it is why the existing
`primitives.spec.tsx` four-combination loop, which looks like coverage, proves only that attributes
can be set.

So the sweep runs in **Chromium via `@storybook/test-runner`**, over the built Storybook, with
`color-contrast` **enabled** - the only place resolved colours and real geometry exist. It also
tests the unit the criterion names: a _story_, not a component in a spec's own harness. Contrast
stays proven in `packages/tokens/src/color.spec.ts`, which pins the pairs; this proves the pairs
**as actually composed on a page**.

**The first run found 40 failures across 49 stories.** Four real defects, none of which any
existing gate could have caught:

1. **The application had no base surface at all.** `layout.tsx` renders a bare `<body>` and
   `globals.css` set no background or foreground, so in dark theme the page was the browser's
   default **white** behind near-white text - `#fafafa` on `#ffffff`, **1.04:1**, eighty times.
   `AppShell` sets `bg-bg-base` on its own wrapper, which is why no screen looked broken and why
   this survived twenty-two tasks. Fixed on `html` so an over-scrolled page does not show white
   past the content, with `color-scheme` so the browser's own scrollbars and caret follow.
2. **`--text-muted` was never contrast-tested.** `color.spec.ts` pinned `textPrimary` and
   `textSecondary` against all four surfaces and simply did not list muted. It measured **3.40:1**
   dark and **3.11:1** light. It carries hints and counters at `text-xs` - body text, 4.5:1, not
   the 3:1 large-text allowance. Recomputed keeping hue and saturation and moving lightness only:
   `#6E6E78` to `#82828D` (worst surface 4.52) and `#8A8A93` to `#6F6F79` (worst 4.51), still
   clearly lighter than secondary so the hierarchy survives. Now pinned against all four surfaces.
3. **`InlineAlert` and `Toast` used `bg-{status}` as a filled surface.** `SB-18` defined exactly
   two tokens per status and `color.spec.ts` pins exactly those two: `--{status}` at 3:1 as a **UI
   boundary**, `--{status}-text` at 4.5:1 as **body text on the base**. `PH-0.27` invented a third
   composition - a filled panel with text on it - that the token system does not have. Measured
   `#18181b` on `#dc2626` at **3.66:1** and `#fafafa` on `#60a5fa` at **2.43:1**. Recomposed to use
   only what was specified: `bg-bg-surface`, the status colour as the border, the status text
   colour on the icon. Filling a panel would need a `--{status}-subtle` token that does not exist,
   and inventing one is a design decision rather than a fix.
4. **`Button variant="danger"` used `--accent-contrast`**, a token computed and pinned against the
   **accent** and qualified for nothing else. On `--danger` it measured 3.67:1 in light and 7.15:1
   in dark - a failure visible in exactly one theme. `--text-inverse` flips per theme and clears
   both, 7.15 and 4.83. Now pinned, along with the fact that accent-contrast fails on danger, so
   the next person who reaches for it finds out why not.

Two further findings that were not contrast at all:

- **`Text tone="accent"` offered the brand colour as body text.** The accent is specified as a 3:1
  boundary colour and measures **3.70:1** on the light base. Changing it is a brand decision and
  `SB-18` settled that the accent hex does not move - so the tone was **removed**. Emphasis in body
  text is weight. The type error found both call sites.
- **`Breadcrumb`'s label defaulted to the literal string `'Breadcrumb'`** - a hardcoded user-facing
  string (`BR-525`) that made a screen reader announce an English word in an Arabic-first product,
  _and_ gave every breadcrumb on a page the same landmark name (`landmark-unique`). The label is
  now required; the type error found nine call sites.

#### 2. Criterion 5, and the count that was wrong

`src/roster.spec.ts` asserts `12 §20.12.1` against what `@josam/ui` actually exports, failing in
both directions. **Its first run found 68, not 69: `Toast` was in the roster and not exported** -
the library shipped `ToastProvider` and `useToast` and no `Toast`. Extracted as a real component
that `ToastProvider` now renders. The 69/69 I reported was a count, and it was wrong.

#### 3. The four unowned items

| Item                               | Now                                                     | Proof            |
| ---------------------------------- | ------------------------------------------------------- | ---------------- |
| `size-limit` (`BR-1486`)           | Installed, budget resolved from the **build manifest**  | Case 38          |
| `pnpm audit` (`BR-1468`)           | `--prod` in CI                                          | `SB-31`          |
| Blanket `"use client"` (`BR-1502`) | `josam/no-blanket-use-client`, scoped to the route tree | Case 39          |
| Ledger rows 10 and 11              | Corrected to done - both were already live and proven   | Cases 18, 19, 23 |

**`size-limit`'s first configuration was wrong and would have been worse than nothing.** The
obvious `chunks/*.js` glob reported **255.63 kB** against a 200 KB budget: it sums every chunk Next
emits, including routes the first paint never touches. The reasonable response would have been to
raise the limit until it passed, leaving a budget that could never mean anything again. Resolved
from `build-manifest.json` instead - shared runtime plus the heaviest single route - it measures
**131.42 kB**, comfortably inside the budget with real headroom.

**The `BR-1502` rule needed its own rule name, and the first attempt would have been a disaster.**
I put the selector in the shared `no-restricted-syntax` array and scoped that rule off outside the
route tree - which would have switched off `BR-855`, `BR-1429` and `BR-1544` across `packages/**`
at the same time, because ESLint **replaces** a rule's options rather than merging them (`SB-16`,
the same mechanism that left three fitness functions dead at `PH-0.16`). A custom rule scopes
cleanly. Its `files` pattern then had to become `**/app/**` rather than `apps/web/app/**`, because
a flat-config pattern resolves against the config's own directory and this config loads from both
the repository root and from inside `apps/web` - the `SB-15` asymmetry again. Fitness case 39
reported **NOT CAUGHT** twice before it reported caught.

#### 4. Redis health indicator - criterion 9, partially

`ioredis` 5.11.1 installed, `REDIS_URL` required in the env schema, `RedisService` and
`RedisHealthIndicator` self-registering into `shared/health` exactly as the database indicator
does, with no import from infrastructure up into `modules/health`.

`SB-16` said this ships with `ioredis` and never "later", and then neither happened: the compose
stack has run Redis since `PH-0.5` and `GET /health` reported `status: ok` while watching one of
two dependencies. `BR-892` names that precisely - worse than never claiming to check it.

**Proven against the real container, both directions**, and the first version failed the second:

```
healthy:        {"status":"ok","checks":{"database":"ok","redis":"ok"}}
redis stopped:  {"status":"degraded","checks":{"database":"ok","redis":"error"}}
redis back:     {"status":"ok","checks":{"database":"ok","redis":"ok"}}
```

The middle line worked immediately. The third did not: my `retryStrategy` returned `null` after
three attempts to limit log noise, and ioredis reads that as _stop reconnecting permanently_, so
after any transient outage the endpoint reported `redis: error` for the life of the process. A
health check that cannot recover is worse than none, because it teaches whoever reads it to ignore
the field. Backoff now caps at 2 s and never gives up.

Queue, storage and `last_backup` stay with their owning phases.

#### 5. Lighthouse CI

Moved from `PH-0.10` to `PH-0.11` in the ledger. It needs a deployed URL, which `PH-0.10` never
had - it was keyed to a task that could not have delivered it.

#### The security audit, and what it cost to make it meaningful

First run: 5 advisories in production dependencies. Three resolved by overriding `postcss` to
8.5.23, **already in the tree** for another consumer while Next pulled 8.4.31 transitively. Two
remain with no safe fix and are **accepted, not ignored**, each recorded in `pnpm-workspace.yaml`
and in `SB-31` with the condition that retires it.
...
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | ---------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~`SB-32`~~ | ~~**`josamacademy.com` is not serving.**~~ **Closed 2026-07-30** — the domain was attached alongside `PH-0.28`. It serves over HTTPS behind a proxied record; all five route groups return 200 from the public domain and the stylesheet is byte-identical through the proxy. The client sites were checked immediately after the proxy reload and were unchanged. One divergence came out of it: two `A` records round-robined, so the domain served a parking page half the time while `curl` reported 200 — the warning is now in `deploy-and-rollback.md §10.1`. | `PH-0.11 §10`                            | 2026-07-30 | Founder                           | ✅ Closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| 2026-08-01 | `16` (header, `§Totals`, Phase 0/1 headings, approval table) | **The stated task count was never derived and was wrong from the first commit.** `16` said **191 tasks** in three places and enumerated **190** at `f6dcc04`. `PH-0.29`, `PH-0.30` and `PH-1.33` each moved the enumeration; the figure never moved. Phase 0's heading read 28 against 30, Phase 1's read 32 against 33. Phases 2–7 were exact. | Nothing recomputed it. At `8e07222` the enumeration rose to meet the claim and the number was **correct by coincidence for one commit** — the failure direction `BR-1849` describes, applied to a count: checking it in that window would have confirmed it. The hypothesis that raised this — duplicated rows hiding a task or double-counting one — was **disproved**: 193 rows, 193 distinct ids, no gaps. The prior "196 / Phase 0 = 34" figures were an AI grep artefact matching an estimate-split discussion table, and are withdrawn. | ✅ **Founder-authorised document edit, 2026-08-01.** All five stale figures corrected to the enumerated **193** / 30 / 33 — including the approval-table row, which was not named in the authorisation but is a third copy of the same number and would have re-seeded the drift. `STATUS.md §1` reconciled. **Enforced from now on** by `check:ledgers`' census section: every phase heading against its rows, every document-wide count against the total, `STATUS §1` per-phase and in total, and `STATUS §1`'s Done column against the machine-checked numerators. Duplicate ids fail. Six deliberate injections, all caught; two kept as fitness cases 50 and 51.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-08-01 | _(none — `scripts/check-ledgers.mjs`)_                       | **The census check's first run miscounted, by reusing the wrong parser.** It enumerated 192 against a true 193, dropping `PH-0.30`, which sits in a **headerless one-row table embedded in prose**.                                                                                                                                             | `parseTaskRows` requires a header row so it can resolve `Status`/`Est` **by name** — correct where those columns matter, a liability where only the id does. Reusing it over-constrained the job and produced a silent blind spot in a check written to eliminate exactly that.                                                                                                                                                                                                                                                               | ✅ **Fixed.** The census has its own id-only scan with an exact-match anchor, which also excludes the discussion-table rows that caused the phantom 196. Caught only because an independent count already existed — **two measurements that disagree is the cheapest defect-finder available, and one measurement is a number nobody can check.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2026-07-31 | `16` (Week 2 — Permissions), `09 §9`, `13 §9`                | **`packages/contracts` had NO owning task.** `16` never created it, in any phase, while `12 §19` row 19 (`BR-1447`), `BR-931` and `BR-1584` all assume it exists — and `09 §9` and `13 §9` both list it in the monorepo tree.                                                                                                                   | Found by widening `check-ledgers`' owner set past Phase 0: row 19 was owned by `PH-1.8`, which is **done** and built the permission registry, not contracts. Re-pointing the row was not available as a fix, because there was nothing to point it at. A ledger row cannot bring its own owner into existence, and the missing task was invisible for as long as the row named a plausible one.                                                                                                                                               | ✅ **Founder-authorised document edit, 2026-07-31.** `PH-1.33` added to `16`'s Week 2 table: `packages/contracts` generated in CI from the NestJS OpenAPI surface (`DEC-30`), hand-edits failing the build. **Depends `1.12`** — generating from the OpenAPI surface requires the surface to exist. **Est 1.5**, sized against `PH-0.16` rather than the 1-day schema tasks. `CLAUDE.md §5b` matches; Phase 1 is now **7 / 33**; `12 §19` row 19 re-owned to `PH-1.33`. The ID is appended rather than inserted, the `PH-0.29`/`PH-0.30` precedent, so no existing task id moves.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2026-07-31 | _(none — `scripts/check-ledgers.mjs`)_                       | **The checker's completion set covered Phase 0 only.** Built from the `PH-0.x` rows, so every **Phase 1** owner marked done passed silently — `12 §19` row 15's original defect, reproduced inside the check written to catch it.                                                                                                               | Two hand-written regexes, one per phase, because `§5` and `§5b` order their columns differently. A per-phase list is correct until a phase starts and quietly wrong afterwards, with the extension being invisible work that nothing demands. It would have reproduced at Phase 2.                                                                                                                                                                                                                                                            | ✅ **Fixed generally.** No phase is named anywhere: any table with an `ID`/`Status` header and `` `PH-N.M` `` in its first column is a task table. Columns resolve **by header name**, not position. Duplicate-id guard added. The rejected alternative — "find the cell with a status glyph" — is already wrong today, because `PH-1.10`'s Note cell carries both 🟡 and ✅ in prose. **The widened checker caught row 19 on its first run.** Proven by fitness case **48**, which points a `12 §19` deferral at a done _Phase 1_ task and reports NOT CAUGHT against the old Phase-0-only owner set. (The `REMOVE-AT` check was never the phase-limited one — it already spanned both tables. Only the deferral owner check was.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2026-07-31 | `16 §PH-1.10`, `16 §PH-1.12`                                 | **`PH-1.10`'s permission guard and `BR-1631` startup check move to `PH-1.12`.** `16` sizes `PH-1.10` at 1 day for "permission guard + scope decorator at the data layer" as one task. The data-layer half is built and proven; the guard half cannot be built.                                                                                  | **The guard needs a route table to attach to, and no route table exists until `PH-1.12`.** Registering it globally today denies every existing endpoint; registering it and annotating nothing authorizes zero requests. Closing `PH-1.10` on that is the `BR-1830` shape — a mechanism that loads and enforces nothing, indistinguishable from a working one in a green gate, because the unit tests instantiate the guard directly and never exercise the wiring.                                                                           | ✅ **Founder-authorised document edit, 2026-07-31** (`BR-1765` — instructed, not taken unilaterally). `16 §PH-1.12`'s row gains both mechanisms, its Est goes **1 → 1.5**, its Output names three proofs instead of one, and its Refs gains `BR-1631`. `CLAUDE.md §5b` matches. **`PH-1.10` stays 🟡 permanently with both gaps named and never reaches ✅** — the `PH-0.9` precedent. `PH-1.10`'s Est stays at **1** against an actual of **0.7**; the undelivered half is carried on `PH-1.12`, not erased by restating the original estimate (`DEC-56`, `BR-1802`). Phase 1's estimated total rises by 0.5.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `SB-45`     | **The 174 permission descriptions are null.** `TBL-008.description` is nullable bilingual JSONB; `05` supplies English text only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **Owner: `PH-1.13`** — the admin permission-matrix editor, the first task that puts these strings in front of a person. Not started (`BR-1840`). **It requires a human Arabic REVIEWER, not a translation pass** — running 174 English strings through a translator reproduces `BR-1851`'s defect with an extra step, because the output is still text nobody who reads the language has approved. Until then the editor shows the permission **key**, which is honest, legible to an operator, and obviously incomplete. `BR-953` would accept a fabrication and no test in this repository can read Arabic, so the null is the only state that stays visible.                                                                                                                                                                                                                                                                                                                                                         | **`PH-1.13`, with a human reviewer**                                                                                         |
| ~~`SB-44`~~ | ~~**Phase 1's progress figure has no automated guard.**~~ **Closed 2026-07-31.** `CLAUDE.md §5b` now carries the 32-row Phase 1 queue, derived from `16` rather than hand-transcribed, and `check:ledgers` recomputes `Progress (Phase 1)` from it.                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Closed by the `REMOVE-AT` work rather than on its own schedule: deciding whether an exception has outlived its reason requires knowing whether the named task is finished, and for a Phase 1 task nothing knew. Both probes pass — numerator and denominator each fail when they drift.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | ✅ Closed                                                                                                                    |
| `SB-43`     | **`/health`'s `version` is set from an environment variable, so it can lie in exactly the situation it exists to detect.** Observed 2026-07-30: `APP_VERSION` was stale in Coolify, so `/health` reported `a4c3d43` while the container was serving `9821098`.                                                                                                                                                                                                                                                                                                                                                                                                                                         | **Recommendation recorded, not yet applied** (founder direction). **Bake the version in at BUILD time** — a Docker `ARG` set from the commit SHA, written into the image — rather than supplying it as deploy-time configuration. The reasoning is the general one: **an audit value supplied by the mechanism it audits cannot detect that mechanism's error.** `version` exists to answer "which code is actually running", and today it answers "which value somebody last typed into the deploy configuration" — which is the very thing in question when the answer matters. Same family as `BR-1830`: a check that reports on itself. This already cost real confusion once at `PH-0.11`, where the field read `0.0.0` for the whole of Phase 0. Apply at the next task that touches the Dockerfile.                                                                                                                                                                                                              | **Next Dockerfile change**                                                                                                   |
| ~~`SB-41`~~ | ~~**Argon2id timing unmeasured on the production CPU.**~~ **Closed 2026-07-30 — measured on the deployed container.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `params m=65536KiB t=3 p=4 · 2 cpus` → **mean 186 ms · median 196 ms · p95 218 ms · min/max 131/218**. Laptop reference **161 ms**. **Server/laptop ratio 1.15×.** Founder decision: **accept as measured, do not retune** — 218 ms at p95 on a login path is acceptable, and the gap is far smaller than the 2-3× feared for a shared 2 vCPU box. **The ratio is the reusable output, not the absolute figure**: 1.15× is the multiplier for every future CPU-bound estimate on this machine, and it says the box is much closer to development hardware than its specification suggests. Note the interaction already in place — `PH-1.4`'s 400 ms enumeration floor exceeds even p95, so the floor still dominates and `BR-1611` holds unchanged.                                                                                                                                                                                                                                                                    | ✅ Closed                                                                                                                    |
| `SB-42`     | **The breach corpus is a 64-entry starter list, not HIBP's full set.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | The mechanism is complete: sharded by 5-hex prefix, parsing HIBP's native `HASH:COUNT` form, so the real corpus drops in with **no code change**. **The gap is now asserted rather than noted.** `password.spec.ts` used to pass identically with 64 hashes or 850 million — the mechanism ran either way, so no test could tell you which corpus you had (`BR-1841`). Three things now make it visible: a `CORPUS_MIN_HASHES` floor of 50 that fails on a truncated file, a `logger.warn` on **every boot** naming the count and the threshold, and a test asserting `productionReady === false`. **That test is inverted deliberately** — asserting `true` would leave the repository permanently red for a deliberate deferral, which teaches people to ignore a failing test. Written this way it is green today and **turns red the moment the real corpus lands**, forcing whoever loads it to flip the expectation and record the real count rather than assume it.                                              | **Before public registration**                                                                                               |
| `SB-40`     | **Paymob activation waits on a commercial registration. Phase 1 is NOT blocked — its final step is.** Paymob's sandbox issues test credentials without the paperwork, so `PH-1.26` builds and tests the full integration; only going live waits.                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **Record it as a step gated, never a phase blocked** — the distinction decides whether Phase 1 starts. **The risk that comes with it:** sandbox webhook payloads sometimes differ from production. `PH-1.26` must state in its own output that its verification was **against sandbox only**, and name activation as where that assumption is finally tested. A payment integration that has only ever seen sandbox traffic is `BR-1830` at the vendor boundary — a mechanism that reports healthy against inputs it chose. The same applies to `PH-1.27` (Fawry) and `PH-1.22` (Stripe test mode). **Tested at the Phase 1 exit criterion — _real money paid and refunded_ — which is the first traffic no sandbox produced.**                                                                                                                                                                                                                                                                                         | **Phase 1 exit**                                                                                                             |
| `SB-39`     | **The container memory limits are applied but unvalidated.** `PH-0.9` measured Josam using **~198 MiB against 3,008 M allocated** — roughly fifteen times headroom — so no limit was approached, let alone reached.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | The execution proved the limits **apply, start, and are read by the processes**. It could not prove they are **correctly sized**, and no next-day idle check can either. They are first exercised when Phase 1 puts real data and traffic behind them. **`josam-postgres` at 1 G with `shared_buffers` 256 MB is the specific one to re-examine** — it is sized for a schema containing one empty migration. **Revisit at `PH-1.15`** — the entitlement engine, the first Phase 1 task carrying a measurable performance criterion (`p95 < 20 ms`, `BR-981`) that undersized `shared_buffers` would actually violate, so the limit is finally tested by something that FAILS rather than by inspection. The premise expires earlier, at **`PH-1.1`**, the first non-empty migration — after that, "sized for an empty schema" simply stops being true. Both have not started (`BR-1840`). Re-check `docker stats` against the limits there, and raise the single container that approaches its ceiling, never all five. | **First Phase 1 task writing real data**                                                                                     |
...
| `SB-35`     | **Certificate expiry is unmonitored and Cloudflare TLS mode is on Automatic.** Two findings from `PH-0.28` execution that are only dangerous together.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Alone, an origin certificate that fails to renew is survivable: SSL mode **Full** encrypts to the origin without validating its certificate, so the site keeps answering. The risk is the pair — **Automatic mode may move to Full (strict)**, which does validate, and an expired or untrusted origin certificate then takes the site down with no warning, because nothing is watching expiry (a paid UptimeRobot feature). Recommended fix removes both at once: install a **Cloudflare Origin CA certificate** (free, 15-year, trusted by Cloudflare) on the origin and pin the mode to **Full (strict)** explicitly. That ends the renewal risk and makes the mode change harmless.                                                                                                                                                                                                                                                                                                                                | **Before Phase 1 traffic**                                                                                                   |
| `SB-34`     | **A degraded `/health` does not alert.** The UptimeRobot monitor is plain HTTP against `josamacademy.com`, not a keyword monitor against `/health` — UptimeRobot only offers the monitor type at creation, and `/health` is not publicly reachable because the API has no domain.                                                                                                                                                                                                                                                                                                                                                                                                                      | **Real, and recorded as a gap rather than as covered.** `/health` returns **200 while degraded** by design — so that a monitoring endpoint is not confused with a down service — which means a failed database, a stopped Redis or a stale backup produces no alert at all. Exit criterion 4 as written (_alerts when the server is stopped_) **is** met and was tested. This is the gap beyond it. Two fixes: expose `/health` publicly, most cleanly as a Next route in `apps/web` that proxies it server-side so the API still needs no domain, then recreate the monitor as **keyword** on `"status":"ok"`; or give the API its own hostname. The first is smaller and adds no exposed surface.                                                                                                                                                                                                                                                                                                                     | **Before Phase 1** — the first real data makes a silent database failure expensive                                           |
...
|  10 | Clickable non-semantic elements               | `BR-1469`            | ✅               | `PH-0.17` activated it; **proven by fitness case 18**. `PH-0.30` correction — the row read ⬜ while the check was live                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|  11 | Missing `aria-label` on icon buttons          | `BR-1471`            | ✅               | `PH-0.20` activated it; **proven by fitness cases 19 and 23**. `PH-0.30` correction                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
|  12 | Array index as a React key                    | `BR-1429`            | ✅ active        | **Was `🟡 configured, UNPROVEN` — corrected 2026-07-30.** Fitness **case 20** injects an index key and asserts `no-restricted-syntax` fires. Proven, not assumed.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|  13 | `transition: all`                             | `BR-1493`            | ✅ active        | `PH-0.16` violation 4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
|  14 | Bundle size budget                            | `BR-1486`            | ✅               | **`PH-0.30`** — `size-limit` installed, budget resolved from the build manifest, case 38. Initial JS 131.42 kB against 200 KB                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|  15 | Core Web Vitals                               | `BR-1486`            | ⬜ **`PH-1.24`** | **Re-owned 2026-07-30** (`SB-37` closed). Was orphaned on `PH-0.11`. Now against `PH-1.24`, the first task that ships a real page — the site renders five **empty** route groups, so a Core Web Vitals score today measures nothing. `PH-1.24` has not started (`BR-1840`).                                                                                                                                                                                                                                                                                                                                               |
|  16 | Contrast ratios                               | `BR-1216`            | ✅ active        | `PH-0.12` pins every ratio; `PH-0.15` runs axe on every story                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|  17 | Blanket `"use client"`                        | `BR-1502`            | ✅               | **`PH-0.30`** — `josam/no-blanket-use-client`, scoped to the route tree, case 39                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|  18 | Unknown or unmaintained dependency            | `BR-1468`            | ✅               | **`PH-0.30`** — `pnpm audit --prod` in CI, two accepted advisories recorded as `SB-31`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|  19 | Endpoint not present in the contract          | `BR-1447`            | ⬜ **`PH-1.33`** | **Re-owned 2026-07-31.** Read `PH-1.8`, which is **DONE** and never created `packages/contracts` — it built the permission registry. The real defect was underneath: **`packages/contracts` had no owning task anywhere in `16`**, while this row, `BR-931` ("the single source of truth for API types") and `BR-1584` ("generated in CI; hand-editing fails the build") all assumed it existed. A row cannot be owned into existence. `PH-1.33` was ADDED to `16` for it (founder-authorised) and depends on `PH-1.12`, because generating from the OpenAPI surface needs that surface to exist. Not started (`BR-1840`) |
|  20 | Scope decorator present                       | `BR-918`             | ⬜ **`PH-1.10`** | **Re-owned 2026-07-30 at the exit reconciliation.** Read `Phase 1` — a phase, not a task, which is what `BR-1833` prohibits: "deferred" without a named task is indistinguishable from forgotten, and no phase can be held to a check. Now against **`PH-1.10`** — permission guard + scope decorator at the data layer, whose own output is _a forgotten `where` cannot leak_, i.e. the check and the task assert the same property. Not started (`BR-1840`). There is still no scoped query and no `_can` until the domain exists                                                                                       |

**`12 §19` score: 17 active and proven · 3 not active.**

Recounted from the table above on 2026-07-30, because **the previous score line disagreed with its own
table** — it read `15 active · 5 deferred` and listed rows 14, 17 and 18 as deferred when `PH-0.30` had
already activated all three. A summary that is not recomputed from its source drifts silently, which is
the whole reason `BR-1832` says re-run rather than re-read. Two rows changed on inspection:

- **Row 12 was marked `configured, UNPROVEN` and is in fact proven** — fitness case 20. The check was
  fine; the ledger was behind it. Understating is the safer direction, but it is still wrong.
- **Row 15 is orphaned** — see `SB-37`. Its owner completed without it.

The three not active — **15** (`PH-1.24`), **19** (`PH-1.8`), **20** (`PH-1.10`) — are now all correctly
deferred under `BR-1833`, and all three name a task that **has not started** (`BR-1840`). What each is
waiting for does not exist yet: real pages to measure, `packages/contracts`, a scoped query.

Row 15 was the exception until 2026-07-30 and is the worked example now recorded beside `BR-1833`.
