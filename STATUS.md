...
| `SB-18` | **Two values in `12 §3.2` fail `BR-1216`.** Light-mode `accentForeground` `#FFFFFF` on `accent` `#A97A18` measures **3.83:1** against the 4.5:1 AA body minimum — that is the primary button. Light-mode `warning` `#CA8A04` on `bgBase` `#FBFBFA` measures **2.84:1** against the 3:1 UI minimum. Dark mode passes both.                                                                                              | Correcting a design specification is a founder decision (`BR-1765`), so no replacement hex was invented. The two ratios are pinned in `packages/tokens/src/color.spec.ts` rather than skipped, so correcting the palette breaks the suite and forces this entry closed.                                                                                                                        | **Before `PH-0.17`** — primitives and all 69 components are built on this palette |
