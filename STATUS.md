...
| **Updated by**     | AI (three founder decisions: `PH-1.10` → `PH-1.12`; `PH-1.33` added for `packages/contracts`; the checker's owner set widened to every phase)                                                                                                                                                 |
...
> **This table is computed by nothing and had drifted in three places** — Phase 0 read 28 against
> `CLAUDE.md §5`'s machine-checked **29**, Phase 1 read 0 against `§5b`'s **7**, and its rows summed
> to 192 against a stated 193. The per-phase figures above are now reconciled against the two
> machine-checked ledgers; Phase 1's denominator is 33 with `PH-1.33` added.
>
> **The 193 is left alone deliberately.** `docs/16` states **191** tasks in two places and contains
> **196** rows matching `` `PH-N.M` ``; the rows above now sum to 193. Three numbers, none derived.
> That census needs a founder-authorised recount (same class as `SB-08`), and picking one here would
> be inventing the answer. **Nothing machine-checks this table** — extending `check:ledgers` to it is
> the obvious follow-up and is not in this session's scope.
