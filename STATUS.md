...
| `SB-44`     | **Phase 1's progress figure has no automated guard.** `check-ledgers.mjs` validates `CLAUDE.md §5`, which is the **Phase 0** queue (`PH-0.\d+`). Phase 1's _N / 32_ is maintained by hand in `STATUS.md` and computed by nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Exposed by the recount, not by the defect. `BR-1832`'s whole argument is that a figure written once and never recomputed drifts — and this one has never been computed at all. It is currently correct because it is small and recent; it will not stay that way across 32 tasks. **Extend `check-ledgers.mjs` to a Phase 1 task table** once one exists in a machine-readable form, or move Phase 1's queue into `CLAUDE.md §5`'s shape so the existing checker covers it. The second is cheaper and keeps one checker rather than two.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **Before Phase 1 exit**                                                                                                      |
