...
| **Updated by**     | AI (`PH-0.28` execution recorded from founder-pasted output)                                                                                                                                                                                                                                  |
| **Current phase**  | Phase 0 — Foundation · **28 / 30**                                                                                                                                                                                                                                                            |
| **Current task**   | _None in progress_ — `PH-0.28` ✅ executed                                                                                                                                                                                                                                                    |
...
| Alert **recovery** notification unconfirmed             | Founder confirming separately                                |
...
**The recovery notification is still unconfirmed and is the one row left open.**
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `SB-37`     | **`12 §19` row 15 — Core Web Vitals — is orphaned.** It was recorded against `PH-0.11` because Lighthouse CI needs a deployed URL. `PH-0.11` closed on 2026-07-30 and did not add it; there is no `lighthouserc` and no Lighthouse step anywhere in the repository.                                                                                                                                                                                                                                                                                                                                                                                                                                    | **Found while reconciling for the exit check, and it is the exact failure `BR-1833` was written to catch** — an enforcement item whose owner has already closed cannot be actioned by anyone, and reads as scheduled rather than as missing. The blocking reason is gone: `josamacademy.com` now serves, so a URL exists to measure. **Founder decision, not an AI one** (`BR-1774`): either it gets a small Phase 0 task before exit, or it is explicitly deferred to a named Phase 1 task. What it must not do is keep pointing at `PH-0.11`.                                                                                                                                                                                                                                                                                  | **Decide before the exit check**                                                                                             |
| `SB-36`     | **The R2 backup credentials were shared into a chat transcript.** Access key ID and secret for `josam-backups`, scoped Object **Read & Write**, no expiry. `.env` is gitignored so nothing reached the repository.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | **Rotate now, not at the Phase 1 boundary.** This is a different exposure from `SB-33`: that pair protects an empty database unreachable from outside the project network, while these reach a bucket on the public internet and the token can both **read every database dump and delete them** — the backup set and its destruction are the same credential. Rotate in Cloudflare (R2 → Manage API tokens), update the environment on **both** the API and backup resources, redeploy, then re-run `backup.sh` by hand and confirm `/health` still reports `last_backup: ok`. Consider a **separate read-only token for the API**, which only lists the bucket.                                                                                                                                                                | **Today**                                                                                                                    |
...
|  15 | Core Web Vitals                               | `BR-1486`            | ⬜ **ORPHANED — `SB-37`** | Was recorded against `PH-0.11`, **which closed on 2026-07-30 without it**. It now points at a done task, which is the same failure `BR-1833` exists to prevent — an owner that can never act. Needs a new owner: founder decision. |
...
The three not active: **15** (orphaned, needs an owner), **19** (`PH-1.8`, needs `packages/contracts`),
**20** (Phase 1, there is no scoped query yet). Rows 19 and 20 are correctly deferred — the thing they
enforce does not exist yet. Row 15 is not: the deployed URL it was waiting for now exists.
